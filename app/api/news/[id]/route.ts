...
export async function GET(
  ...
  { params }: { params: Promise<{ id: string }> } // tetap pakai Promise
) {
  try {
    const paramsValue = await params; // resolve promise dulu
...
export async function PUT(
  ...
  try {
    ...
    const news = await News.findByIdAndUpdate(
      id,
      { date, category, title, description, content, status },
      { new: true, runValidators: true }
    );

    if (!news) return createErrorResponse("NOT_FOUND", "ニュースが見つかりません", 404);
...
export async function DELETE(
  ...
  try {
    ...
    return createSuccessResponse({ message: "ニュースを削除しました" });
  } catch (error: any) {
